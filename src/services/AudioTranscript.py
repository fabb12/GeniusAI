...
class TranscriptionThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if duration < 30:
                # Transcribe short audio in a single pass
                self.progress.emit(50, "Trascrizione file breve...")
                text = self.transcribe_full_audio(audio_file, language_video)
                transcription = f"[00:00]\n{text}\n\n"
                self.progress.emit(100, "Trascrizione completata.")
            else:
                # Use chunking for longer audio
                length = 30000  # 30 secondi in millisecondi
                chunks = [
                    (audio_clip.subclip(start / 1000, min((start + length) / 1000, duration)), start)
    ...
    def convertVideoToAudio(self, video_file, audio_format='wav'):
        logging.debug("Checkpoint: Inside convertVideoToAudio")
        video = VideoFileClip(video_file)
        audio = video.audio
        with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as temp_audio_file:
            audio.write_audiofile(temp_audio_file.name, codec='pcm_s16le', logger=None)
    ...
    def get_locale_from_language(self, language_code):
        logging.debug(f"Checkpoint: Converting language code {language_code} to locale")
        try:
            ...
            logging.debug(f"Checkpoint: Exception in get_locale_from_language - {e}")
            return language_code

    def transcribe_full_audio(self, audio_path, language_code):
        recognizer = sr.Recognizer()
        with sr.AudioFile(audio_path) as source:
            audio_data = recognizer.record(source)
            locale = self.get_locale_from_language(language_code)
            try:
                ...
                return f"[Errore di richiesta: {e}]"
            except Exception as e:
                return f"[Errore generico: {e}]"

    def transcribeAudioChunk(self, audio_chunk, start_time):
        logging.debug("Checkpoint: Inside transcribeAudioChunk")
        recognizer = sr.Recognizer()
        try:
            with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as temp_audio_file:
                audio_chunk.write_audiofile(temp_audio_file.name, codec='pcm_s16le', logger=None)
                temp_audio_file_path = temp_audio_file.name
