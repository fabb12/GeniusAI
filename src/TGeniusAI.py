...
class VideoAudioManager(QMainWindow):
    ...
    def initUI(self):
        ...
        for text_edit in all_custom_text_edits:
            if text_edit:
                # If the widget is a RichTextEditor, connect to its inner text_edit
                if isinstance(text_edit, RichTextEditor):
                    text_edit.fontSizeChanged.connect(self.handle_font_size_change)
                else: # Otherwise, it's a CustomTextEdit, connect directly
                    text_edit.fontSizeChanged.connect(self.handle_font_size_change)
    ...
    def handle_font_size_change(self, new_size):
        ...
        """
        if not self.ui_initialized or self._is_applying_font_settings:
            return
